...
describe('BooksComponent', () => {
    ...
    test('creates Books with a Book', () => {
        ...
        const book = books.children.item(0);
    ...
    test('throws an error for Books with no Book', () => {
        expect(() => BooksComponent()).toThrowError(`BooksComponent could not be created. There must be one fallback Book (does not have a 'media' attribute defined).`);
    });
    test('throws an error for Books with a Book but no fallback', () => {
        expect(() => BooksComponent({
            children: [BookComponent(pagesOutput, { media: { minWidth: 600 } })],
        })).toThrowError(`BooksComponent could not be created. There must be one fallback Book (does not have a 'media' attribute defined).`);
    });
});
